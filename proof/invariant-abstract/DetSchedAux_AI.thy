...
lemma dmo_getCurrentTime_sp[wp]:
  "do_machine_op getCurrentTime \<lbrace>P\<rbrace> \<Longrightarrow>
   ...
   do_machine_op getCurrentTime
   \<lbrace>\<lambda>rv s. (cur_time s \<le> rv) \<and> (rv \<le> - (kernelWCET_ticks + 3 * MAX_PERIOD) - 1) \<and> P s\<rbrace>"
  apply (clarsimp simp: pred_conj_def)
  apply (intro hoare_vcg_conj_lift_left_fix)
    apply (rule_tac Q="valid_machine_time" in hoare_weaken_pre)
     apply (rule_tac Q="\<lambda>rv s. cur_time s \<le> rv \<and> rv \<le> - (kernelWCET_ticks + 3 * MAX_PERIOD) - 1"
...
lemma dmo_getCurrentTime_wp:
  assumes str_post: "\<And>rv s. (cur_time s \<le> rv) \<and> (rv \<le> - (kernelWCET_ticks + 3 * MAX_PERIOD) - 1)
                             \<and> P s
...
lemma update_time_stamp_cur_time_monotonic:
  ...
  apply (rule_tac B="\<lambda>rv s. cur_time s \<le> rv \<and> rv \<le> - (kernelWCET_ticks + 3 * MAX_PERIOD) - 1
