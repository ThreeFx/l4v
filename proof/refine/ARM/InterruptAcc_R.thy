...
lemma refills_heads_equal_active:
  "\<lbrakk>sc_refills sc \<noteq> []; valid_sched_context' sc' s'; \<exists>n. sc_relation sc n sc'\<rbrakk>
   \<Longrightarrow> rAmount (refillHd sc') = r_amount (refill_hd sc) \<and> rTime (refillHd sc') = r_time (refill_hd sc)"
...
crunches update_time_stamp
  ...
  (simp: do_machine_op_def cur_sc_tcb_def)
...
definition "getCurrentTime_independent_D (P :: kernel_state \<Rightarrow> bool)
...
lemma getCurrentTime_independent_DI[intro!, simp]:
   "\<lbrakk>\<And>s f.
     P (s\<lparr>ksMachineState
          := (ksMachineState s)\<lparr>last_machine_time :=
                                f (last_machine_time (ksMachineState s)) (time_state (ksMachineState s))\<rparr>\<rparr>)
     = P s\<rbrakk>
    \<Longrightarrow> getCurrentTime_independent_D P"
   by (simp add: getCurrentTime_independent_D_def)

definition "time_state_independent_D (P :: kernel_state \<Rightarrow> bool)
  \<equiv> \<forall>f s. P s \<longrightarrow>
           P (s\<lparr>ksMachineState := ksMachineState s\<lparr>time_state := f (time_state (ksMachineState s))\<rparr>\<rparr>)"

lemma time_state_independent_DI[intro!, simp]:
   "\<lbrakk>\<And>s f. P (s\<lparr>ksMachineState := ksMachineState s\<lparr>time_state := f (time_state (ksMachineState s))\<rparr>\<rparr>)
            = P s\<rbrakk>
    \<Longrightarrow> time_state_independent_D P"
   by (simp add: time_state_independent_D_def)

lemma updateTimeStamp_inv:
   "\<lbrakk>updateTimeStamp_independent P; time_state_independent_D P; getCurrentTime_independent_D P\<rbrakk>
  ...
  apply (rule hoare_seq_ext_skip, wpsimp)
   apply (fastforce simp: time_state_independent_D_def getCurrentTime_independent_D_def in_monad)
...
lemma preemptionPoint_inv:
  assumes "(\<And>f s. P (ksWorkUnitsCompleted_update f s) = P s)"
          ...
          "getCurrentTime_independent_D P"
          "time_state_independent_D P"
  shows "preemptionPoint \<lbrace>P\<rbrace>" using assms
  apply (simp add: preemptionPoint_def setWorkUnits_def getWorkUnits_def modifyWorkUnits_def
                   isCurDomainExpired_def getDomainTime_def setConsumedTime_def
                   setCurTime_def refillSufficient_def)
