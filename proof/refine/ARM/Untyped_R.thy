...
lemma reset_untyped_cap_corres:
  ...
  apply (rule corres_guard_imp)
   ...
   subgoal by (intro conjI impI; auto simp: valid_sched_def)
    ...
    rule_tac P="\<lambda>i. invs' and ?psp and ct_active' and valid_untyped_inv_wcap' ?ui
     ...
     apply (rule hoare_pre)
      ...
      apply (wpsimp wp: preemptionPoint_invs
              ...
              | simp add: ct_in_state'_def getCurrentTime_independent_D_def ex_cte_cap_wp_to'_def
                          time_state_independent_D_def sch_act_simple_def
