...
lemma refills_heads_equal:
  ...
  apply (frule hd_refills_map[where mx="scRefillMax sc'" and start="scRefillHead sc'"
                                and count="scRefillCount sc'", rotated]
         ; simp?)
   apply (frule hd_wrap_slice[where mx="scRefillMax sc'" and list="scRefills sc'"]
          ; simp?)
   apply (clarsimp simp: refills_map_def)
...
lemma awakenBody_release_queue_length:
  "\<lbrace>\<lambda>s'. the (releaseQNonEmptyAndReady s') \<and> s' = s\<rbrace>
   awakenBody
   \<lbrace>\<lambda>r' s'. length (ksReleaseQueue s') < length (ksReleaseQueue s)\<rbrace>"
...
lemma awaken_terminates:
  ...
  apply (rule_tac R="{((r', s'), (r, s)). length (ksReleaseQueue s') < length (ksReleaseQueue s)}"
    ...
    apply simp
   apply clarsimp
   apply (rule awakenBody_release_queue_length)
...
lemma awakenBody_corres:
  "corres dc ((pspace_aligned and pspace_distinct and valid_objs and valid_sched_action
              ...
              and (\<lambda>s. Structures_A.release_queue s \<noteq> []))
  ...
  apply (rule_tac Q="\<lambda>rv s. ?pred s \<and> tcb_at rv s"
              and Q'="\<lambda>rv s. ?conc s \<and> st_tcb_at' runnable' rv s"
