...
lemma in_preempt':
  ...
  apply (clarsimp simp: alternative_def in_monad eq_commute getActiveIRQ_def doMachineOp_def
   ...
   apply (cases "workUnitsLimit \<le> ksWorkUnitsCompleted s + 1")
    ...
    apply clarsimp
    apply (rule_tac x="of_nat (min (18446744073709551615 - (unat kernelWCET_ticks + 3 * unat MAX_PERIOD))
    ...
    apply (rule_tac x="ksConsumedTime s
                       + of_nat (min (18446744073709551615 - (unat kernelWCET_ticks + 3 * unat MAX_PERIOD))
                                     (unat (last_machine_time (ksMachineState s))
                                      + time_oracle (Suc (time_state (ksMachineState s)))))
                                      - ksCurTime s"
                 in exI)
...
lemma not_recursive_ctes_independent_simple[simp]:
   "not_recursive_ctes (s \<lparr> ksCurTime := t \<rparr>) = not_recursive_ctes s"
   "not_recursive_ctes (s \<lparr> ksConsumedTime := t' \<rparr>) = not_recursive_ctes s"
   "not_recursive_ctes (s \<lparr> ksMachineState := t'' \<rparr>) = not_recursive_ctes s"
   by (simp add: not_recursive_ctes_def)+
...
definition
  "finalise_prop_stuff P
    ...
    \<and> getCurrentTime_independent_D P
    \<and> time_state_independent_D P
...
lemmas reduceZombie_invs'
    = reduceZombie_invs''[where Q=\<top>, simplified no_cte_prop_top simp_thms
         ...
         getCurrentTime_independent_D_def time_state_independent_D_def,
     ...
     apply wp
        ...
        apply (wp "1.hyps")
          ...
          apply (wp preemptionPoint_inv)[1]
           ...
           apply (clarsimp simp: getCurrentTime_independent_D_def)
          apply (clarsimp simp: time_state_independent_D_def)
         apply (wp capSwap_rvk_prog | simp only: withoutPreemption_def)+
